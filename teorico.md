...
// Resposta
/**
 * Considero a melhor maneira a includsão de imagens utilizando a tag <img/>
 * pois para performace os navegadores ja estão implementando lazyload nativo
 * e podemos ter um texto aternativo para acessibilidade.
 * 
```
...
// Resposta
/**
 * Conheço o padrão BEM que é uma metodologia para criação de nomes para classe de folha de estilo
 * ex: .elementopai, .elementopai__filho, .elementopai__filho- -primeiro
 * 
 * A grande ideia é padronizar para que qualquer front-end, ao se deparar com essa estrutura, separada por “__” ou “- -“, 
 * identifique os elementos e os modificadores só de analisar um HTML.
 * Utilizando SASS facilita a estilização
 * 
 * ex: .elementopai {
 *          &__filho {
 *              &--primeiro {
...
// Resposta
/**
 * Costumo utilizar a grid do bootstrap e/ou flexbox
 * não tem nenhum que não achei interessanta ainda
```
...
// Resposta
/**
 * O estilo da classe '.list-item-holder ul li' será sobrescrito pelo estilo desta classe '.list-item-holder .item' pois estão referenciando ao mesmo elemento
 * No estilo '.list-item-holder .item .item__input-radio' o border onrange é sobreescrita para o border #eadabd
 * O style da classe '.item__input-radio--cor-diferente' não é aplicado
 * no media query o slyle do '.item__input-radio--cor-diferente' não é aplicado
```
